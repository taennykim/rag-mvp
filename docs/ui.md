...
- `/upload`에서는 parser 선택, chunk 설정, 업로드, parse test, quality check, indexing 상태 확인이 가능하다.
...
## 4. 이슈 및 문제
- `Uploaded file list`의 parse success/failure history UI는 구현이 진행됐지만 최종 화면 검증이 아직 남아 있다.
...
## 5. 다음 작업
- upload 화면의 parse history 표시를 실제 화면 기준으로 검증하고 필요 시 정리한다.
- `/chat`에 answer panel과 citation 표시를 추가한다.
- `/chat`에 retrieval context 기반 LLM answer generation을 연결한다.
