...
- `/chat` Search API payload는 현재 `query`, `top_k`, `final_k`, 조건부 `filters.product_name`, `return_format=json`, query rewrite 기반 `keyword_vector_weight`만 사용하며 `filters.document_type`, `chunk_types`, `filters.year`는 보내지 않는다.
