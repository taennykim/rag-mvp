...
### 채택 모델
- Query Rewrite 기본 LLM은 `gpt-4o-mini`이고, UI에서 `gpt-4o` 등 다른 deployment를 선택할 수 있다.

### 권장 설정
- model: `gpt-4o-mini` 기본, 필요 시 `gpt-4o`
- temperature: `0.3`
- max_tokens: `700`
...
- score 기준 정렬 확인
- top_k 제한 적용
- Search API 결과의 `document_name`, `header_path`, `content/text`, `scores.rrf_score`는 answer generation과 UI 표시까지 유지한다.
- 화면의 `Reference context`는 최종 정렬된 결과를 그대로 사용하며, 우선순위는 `rrf_score desc -> rerank_score desc -> score desc -> 기존 순서`다.
- 현재 `/chat` Search API payload는 `query`, `top_k=30`, `final_k=10`, 조건부 `filters.product_name_tokens`, `return_format=json`, query rewrite 기반 `keyword_vector_weight`를 사용한다.
- 현재 `/chat` Search API payload에는 `filters.document_type`, `chunk_types`, `filters.year`를 보내지 않는다. document type은 rewrite trace/routing hint로만 유지한다.
...
- `document_name`, `header_path`, `contents`, `rrf_score`를 내부 `hits` / `retrieved_chunks`에 유지한다.
- metadata가 없으면 fallback 문자열을 사용한다.
...
### Answer Generation 전 context 정제
- Query Rewrite 결과 또는 최종 retrieval query에서 상품명/보험명 후보를 추출한다.
- Search 결과의 `document_name`과 비교해, 질의 상품명과 명백히 다른 상품 문서라면 Answer prompt에서만 제외한다.
- 비교가 애매하거나 질의에 상품명/보험명이 없으면 제외하지 않는다.
- `법인계약건`, `제지급`, `자동이체` 같은 업무/처리 유형 키워드는 상품명 후보로 과잉 해석하지 않아야 한다.
- 필터 적용 후 answer context가 비면 원본 Search 결과로 fallback 하는 보강이 필요하다.
- UI의 `Reference context`는 원본 검색 결과를 유지하고, Answer prompt 직전에만 필터를 적용한다.
...
현재 구현 우선순위에서는 제외했고, 추후 개발 단계에서 진행한다.
